...
- **Architecture:** 4-layer hexagonal (Domain → Core → Infrastructure → WebApp)
...
Domain Layer (src/Domain/)
    ↓ (depends on nothing)
Core Layer (src/Core/) [ports/interfaces]
    ↓
...
- Used for Core-level responses, not entity serialization

**See also:** [Readme.md > src/Core/](Readme.md#-src-Core)

### 🟢 Infrastructure Layer (`src/Infrastructure/`)

Implements ports from Core. Contains all adapters for data, caching, messaging, and telemetry.
...
public sealed class OrderEndpoints
{
    ...
    public static void MapOrderEndpoints(this WebCore app)
    {
...
- `src/Core/CoreDependencyInjection.cs` — Auto-registers use cases, validators
...
- `src/Core/Common/Messages/` — Message record contracts
...
**Testing:**
- `tests/CommonTests/Fixtures/` — Shared base fixtures (`BaseCoreFixture`, `BaseHttpFixture`, `BaseMessagingFixture`)
- `tests/UnitTests/Domain/` — Domain entity and value object tests
- `tests/UnitTests/Core/` — Use case and validator tests
...
### Test Structure
```
tests/UnitTests/              # Isolated domain/Core tests
tests/IntegrationTests/       # Full HTTP slice tests + real DB
tests/LoadTests/              # k6 performance scripts
tests/CommonTests/            # Shared fixtures (BaseCoreFixture<T>)
```
...
**Base fixture:** Inherit from `BaseCoreFixture<TRequest, TUseCase>` which provides:
...
[Collection("WebCoreFactoryCollectionDefinition")]
public sealed class CreateOrderTest : IClassFixture<CreateOrderTestFixture>
{
    private readonly CreateOrderTestFixture _fixture;

    public CreateOrderTest(CustomWebCoreFactory<Program> customWebCoreFactory, CreateOrderTestFixture fixture)
    {
        _fixture = fixture;
        _fixture.SetApiHelper(customWebCoreFactory);
...
Integration tests run against a real `WebCoreFactory<Program>` with real PostgreSQL database. Tests verify the full request → response flow across all layers.

#### HTTP Integration Tests

**Structure:**
- **File location:** `tests/IntegrationTests/WebApp/Http/{Feature}/{Operation}Test.cs`
- **Base fixture:** Inherit from `IClassFixture<BaseHttpFixture>` with `[Collection("WebCoreFactoryCollectionDefinition")]`
...
public class GetOrderTest : IClassFixture<BaseHttpFixture>
{
    ...
    public GetOrderTest(CustomWebCoreFactory<Program> customWebCoreFactory, BaseHttpFixture fixture)
    {
        _fixture = fixture;
        _fixture.SetApiHelper(customWebCoreFactory);
...
{
    // Implement Handle to call Core use case
...
- **Base fixture:** Inherit from `BaseMessagingFixture` with `[Collection("WebCoreFactoryCollectionDefinition")]`
...
public sealed class CreateNotificationTest : IClassFixture<CreateNotificationTestFixture>
{
    ...
    public CreateNotificationTest(CustomWebCoreFactory<Program> factory, CreateNotificationTestFixture fixture)
    {
        _fixture = fixture;
...
{
    public new void SetServices(CustomWebCoreFactory<Program> factory)
    {
...
- `stryker-config-Core.json` — validates Core layer tests catch mutations
- `stryker-config-Core.json` — validates Domain layer tests catch mutations
...
- **Environment variables:**
  ...
  - `WEBAPP_URL` — Core URL (default: https://localhost:7175)
...
**Start Core and backing services:**
```bash
...
1. Define port interface in Core layer (e.g., `IMyService`) under `Common/Services/`
...
Always follow the dependency direction: **Domain → Core → Infrastructure → WebApp → Tests**.

### Step 1: Domain Entity
- **File:** `src/Domain/{Feature}/{Entity}.cs`
- **Pattern:** Factory method with invariant validation, sealed class
- **No external dependencies** — only Core.Common references allowed
...
- **Files:**
  - `src/Core/{Feature}/{OperationName}UseCase.cs` (use case + request + validator)
  - `src/Core/{Feature}/{Entity}Dto.cs` (response DTO)
...
- **Core tests:** `tests/UnitTests/Core/{Feature}/{Operation}UseCaseTests.cs`
  - Test with mocked repository via `BaseCoreFixture`
...
- **Integration tests:** `tests/IntegrationTests/WebApp/Http/{Feature}/{Operation}Test.cs`
  - Test against real database via `WebCoreFactory`
...
| **Use Case Generator** | User requests use case; asks to implement CRUD/Delete operations; mentions `BaseInOutUseCase` or `BaseInUseCase` | Generate Core use cases with request records, FluentValidation validators, repository calls, messaging patterns, and xUnit tests |
...
| **Integration Test Generator** | User asks for integration tests for HTTP/gRPC/messaging; wants WebCoreFactory-based tests; requests end-to-end validation | Generate HTTP/gRPC/messaging integration tests with proper fixtures, realistic scenarios, assertions, deduplication testing, and database verification |
...
- **Dependency direction:** Always inward (Domain → Core → Infrastructure → WebApp)
- **Single Responsibility:** Each class has one reason to change
- **Ports pattern:** Infrastructure implements Core-defined interfaces only
...
- **Fixture patterns:** Use `BaseCoreFixture<TRequest, TUseCase>` for mocked tests, `BaseHttpFixture` for HTTP tests, `BaseMessagingFixture` for messaging tests
...
- **Records for DTOs:** Immutable record types for Core responses

### Security Standards
- **No hardcoded credentials:** All config via environment variables or configuration
- **Input validation:** Validate all requests at Core layer boundary
